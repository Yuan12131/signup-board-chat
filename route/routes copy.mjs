...
async function getMessagesByRoomId(roomId) {
  const selectQuery = 'SELECT * FROM messages WHERE roomId = ? ORDER BY timestamp;';
...
const attachSocketEvents = (io) => {
  ...
  io.on("connection", (socket) => {
    ...
    // 클라이언트로부터 이전 메시지 요청을 받았을 때
socket.on('loadMessages', async (data) => {
  const { roomId } = data;

  try {
    // 해당 방의 이전 메시지를 데이터베이스에서 가져오기
    ...
    console.error('Error loading messages:', error);
    // 에러가 발생한 경우 클라이언트에 에러 메시지 전송
    socket.emit('messageError', { message: 'Error loading messages' });
  }
});
